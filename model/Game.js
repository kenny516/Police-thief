...
//east
        ...
        this.sommets = [cn, cs, cw, ce, cc, nn, ns, nw, ne, sn, ss, sw, se, wn, ws, ww, we, en, es, ew, ee]
        if (thief) {
            this._thief = thief;
    ...
    initial_paint() {
        this.personnes.forEach(function (personne) {
            const classeAncienne = document.getElementsByClassName(personne.place.id)[0];
            if (classeAncienne) {
                if (personne.type === 'V') {
